...
def draw_predictions(input: dict, output: dict, mean=0.0, std=1.0, max_images=32):
    ...
    for i in range(num_images):
        ...
        true_label = int(input[INPUT_TRUE_LABEL][i])
        pred_label = int(output[OUTPUT_PRED_LOGITS][i].argmax())
        ...
        cv2.putText(header, f"{true_label}/{pred_label}", (10, 30), cv2.FONT_HERSHEY_PLAIN, 1, (250, 250, 250))
